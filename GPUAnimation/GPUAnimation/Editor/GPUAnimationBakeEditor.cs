...
namespace GPUAnimationLib
{
    ...
    public class GPUAnimationBakeEditor : EditorWindow
    {

        [MenuItem("GPUAnimation/GPUAnimationBake_SkinMesh")]
        private static void ShowWindow()
        {
            var window = GetWindow<GPUAnimationBakeEditor>();
            window.titleContent = new GUIContent("GPUAnimationBake");
            window.Show();
        }
        ...
        private void OnEnable()
        {
            ...
            m_URPShader = DetectPipeline() != PipelineType.Default;
        }
        ...
        private void OnGUI()
        {
            EditorGUILayout.HelpBox("FBX不要开启:Optimize GameObjects,避免Bone骨骼对象优化掉", MessageType.Warning);
        ...
        private void DrawGUI()
        {
            ...
            EditorGUILayout.LabelField("FBX实例对象");
            ...
            else
            {
                ...
                if (GUILayout.Button("烘焙骨骼动画"))
                {
                    ...
                    else
                    {
                        ...
                        {
                            Msg($"FBX不要开启:optimizeGameObjects");
                            return;
                        }
        ...
        void GenerateVertexTexture(GameObject _targetFBX, AnimationClip[] _clips)
        {
            ...
            var instantiatedObj = GameObject.Instantiate(_targetFBX) as GameObject;
            ...
            string pathname = string.Format("{0}{1}{2}_GPU_Vertex{3}{4}", savePath, meshName, (m_URPShader ? "_URP" : ""),m_colorMode.ToString(), (m_BakeNormal ? "_Lit.asset" : "_UnLit.asset"));
        ...
        void GenerateBoneInstanceMeshAndTexture(GameObject _targetFBX, AnimationClip[] _clips, string exposeBones)
        {
            ...
            try
            {
                ...
                atlasTexture.name = meshName + "_AnimationAtlas";
                instanceMesh.name = meshName + "_InstanceMesh";
                string pathname = string.Format("{0}{1}{2}_GPU_Bone{3}{4}", savePath, meshName, (m_URPShader ? "_URP" : ""),m_colorMode.ToString(), (m_BakeNormal ? "_Lit.asset" : "_UnLit.asset"));
        ...
        private bool SelectDirectory(UnityEngine.Object _srcAsset, out string directoryPath, out string objName)
        {
            ...
            if (folderPath.Length == 0)
                return false;
            directoryPath = FileToAssetPath(folderPath) + "/";
