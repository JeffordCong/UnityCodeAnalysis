...
// All Right Reserved.
//------------------------------------------------------------------------------
#ifndef GPUANIMATIONCOMMON_RGBM_CG
#define GPUANIMATIONCOMMON_RGBM_CG
...
// float4x4 SampleTransformMatrix1(uint sampleFrame,uint transformIndex)
// {
...
//         {
//             int pixelX = transformIndex * 12 + row * 4 + col;
//             int pixelY = sampleFrame;
//            // float2 uv = float2((pixelX + 0.5) / texWidth, (pixelY + 0.5) / texHeight);
//             float4 uv = float4(pixelX + 0.5,pixelY+0.5,0,0);
//             float4 c =tex2Dlod(_AnimTex, uv * _AnimTex_TexelSize);
//             float v = DecodeFloatRGBA32(c, _BoundsRange.x, _BoundsRange.y);
//             m[row][col] = v;         
//         }          
//     }        
//     return m;        
...
float4x4 SampleTransformMatrix(uint sampleFrame, uint transformIndex)
{
    ...
    // 第一行
    float4 uv00 = float4(basePixelX + 0.5, sampleFrame + 0.5, 0, 0);
    float4 uv01 = float4(basePixelX + 1.5, sampleFrame + 0.5, 0, 0);
    float4 uv02 = float4(basePixelX + 2.5, sampleFrame + 0.5, 0, 0);
    float4 uv03 = float4(basePixelX + 3.5, sampleFrame + 0.5, 0, 0);
    m[0][0] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv00 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[0][1] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv01 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[0][2] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv02 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[0][3] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv03 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    
    // 第二行
    float4 uv10 = float4(basePixelX + 4.5, sampleFrame + 0.5, 0, 0);
    float4 uv11 = float4(basePixelX + 5.5, sampleFrame + 0.5, 0, 0);
    float4 uv12 = float4(basePixelX + 6.5, sampleFrame + 0.5, 0, 0);
    float4 uv13 = float4(basePixelX + 7.5, sampleFrame + 0.5, 0, 0);
    m[1][0] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv10 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[1][1] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv11 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[1][2] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv12 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[1][3] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv13 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    
    // 第三行
    float4 uv20 = float4(basePixelX + 8.5, sampleFrame + 0.5, 0, 0);
    float4 uv21 = float4(basePixelX + 9.5, sampleFrame + 0.5, 0, 0);
    float4 uv22 = float4(basePixelX + 10.5, sampleFrame + 0.5, 0, 0);
    float4 uv23 = float4(basePixelX + 11.5, sampleFrame + 0.5, 0, 0);
    m[2][0] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv20 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[2][1] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv21 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[2][2] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv22 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
    m[2][3] = DecodeFloatRGBA32(tex2Dlod(_AnimTex, uv23 * _AnimTex_TexelSize), _BoundsRange.x, _BoundsRange.y);
...
float3 SamplePositionPowOf2(uint vertexID, uint frame, uint offset)
{
	...
	float4 encodedData = tex2Dlod(_AnimTex, float4(animUv, 0, 0));
...
float3 SamplePosition(uint vertexID, uint frame, uint offset)
{
    ...
    float4 encodedData = tex2Dlod(_AnimTex, float4((vertexID * offset + .5) * _AnimTex_TexelSize.x, frame * _AnimTex_TexelSize.y, 0, 0));
    float3 normalizedPos = DecodeRGBM(encodedData);
...
float3 SampleNormal(uint vertexID, uint frame, uint offset)
{
    ...
    float4 encodedData = tex2Dlod(_AnimTex, float4((vertexID * offset + 1 + .5) * _AnimTex_TexelSize.x, frame * _AnimTex_TexelSize.y, 0, 0));
    float3 normalizedPos = DecodeRGBM(encodedData);
    float3 worldPos = normalizedPos * 2.0 - 1.0; // 从[0,1]恢复到[-1,1]    
    return worldPos;
}
